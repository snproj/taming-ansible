...
updateWhen t f = case t of
    Atomic {} -> t {
        atomicAttributeSet = (atomicAttributeSet t) {
            atomicWhen = f (atomicWhen (atomicAttributeSet t))
        }
    }
    Blocktask {} -> t {
        block = (block t) {
            blockMain = case blockMain (block t) of
                [] -> error "ERROR: blockMain must not be empty!"
                b:bs -> updateWhen b f : bs,
...
drawFailureArrows :: [Task] -> Task -> Task
drawFailureArrows ts t = updateWhen t (\w -> JBE_EXP_BINARYOP w JBE_OP_AND (orTogether (map getFailureIndicator ts)))
    where
        orTogether :: [JBE_EXP] -> JBE_EXP
        orTogether = foldl1 (`JBE_EXP_BINARYOP` JBE_OP_OR)

chainTogether :: [Task] -> [Task]
chainTogether ts = case ts of
    [] -> []
    [s] -> [s]
    x:xs -> x : zipWith drawSuccessArrow ts xs

drawArrowsInBlock :: Task -> Task
drawArrowsInBlock t = t {
        block = b {
            blockMain = chainTogether (blockMain b),
            rescue = chainTogether newRescue,
            always = chainTogether (always b)
        }
    }
    where
        b = block t
        newRescue = case rescue b of
            [] -> []
            r -> drawFailureArrows (blockMain b) (head r) : tail r

propagateBAS :: Task -> Reader BlockAttributeSet Task
propagateBAS t = case t of
    Atomic {} -> do
        a <- ask
        return t {
        atomicAttributeSet = (atomicAttributeSet t) {
            atomicNotify = atomicNotify (atomicAttributeSet t) ++ blockNotify a,
            atomicVars = Data.Map.union (atomicVars (atomicAttributeSet t)) (blockVars a)
        }
    }
    Blocktask {} -> do
        a <- ask
        let a' = a {
            blockNotify = blockNotify (blockAttributeSet t) ++ blockNotify a,
            blockVars = Data.Map.union (blockVars (blockAttributeSet t)) (blockVars a)
        }
        bm <- traverse (local (const a') . propagateBAS) (blockMain (block t))
        r <- traverse (local (const a') . propagateBAS) (rescue (block t))
        al <- traverse (local (const a') . propagateBAS) (always (block t))
        return t {
            block = (block t) {
                blockMain = bm,
                rescue = r,
...
setIgnoreError :: Task -> Task
setIgnoreError t = case t of
    Atomic {} -> t {
        atomicAttributeSet = (atomicAttributeSet t) {
            atomicIgnoreErrors = True
        }
    }
    Blocktask {} -> t {
        block = (block t) {
...
-- getSuccessIndicator (Blocktask _ block _) = let
